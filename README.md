...
연월(YYYYMM)을 지정해 **서울 전역(또는 한 구)의 상업업무용 통건물 실거래를 CSV 파일**로
내려받습니다. 시세를 화면으로 보는 게 아니라 **데이터 자체를 파일로 갖고 싶을 때** 씁니다.

**사용방법**

```
/deals-export 202607                → 서울 25개 구 전체, 2026년 7월 한 달
/deals-export 202601-202606 강남구  → 강남구 상반기(범위는 최대 24개월)
/deals-export 202607 역매칭         → 마스킹 지번을 표제부 역매칭으로 복원해 포함
```

> 직접 호출: `deals_export(year_month="202607")`,
> 범위·구 한정은 `deals_export(year_month="202601", year_month_to="202606", gu="강남구")`

**결과물** — `~/deal-locator-exports/<날짜>/실거래_통건물_서울전역_202607.csv`
(utf-8-sig — 엑셀에서 바로 열립니다. `DEAL_LOCATOR_EXPORT_DIR` 로 폴더 변경 가능)

```
■ 실거래 CSV 내보내기 — 서울전역 202607 (통건물)
  파일: ~/deal-locator-exports/2026-08-17/실거래_통건물_서울전역_202607.csv
  행수: 202건 (해제신고 8건 포함·컬럼으로 구분)
  ※ 집합(구분상가) 588건은 취급 범위 밖이라 제외(v1)
  ※ 지번 마스킹 202건 — 역매칭 없이 원본 그대로(match=true 로 복원 가능)
...
- **기본은 국토부 원본 그대로**라 지번이 `6**` 처럼 마스킹돼 있습니다. `match=true`
  (슬래시 명령에서는 `역매칭`)를 붙이면 `대지위치_표제부`·`매칭단계` 컬럼으로 복원
  지번이 채워집니다 — 단, 동별 건축물대장을 받아야 해서 콜드 캐시면 수 분~수십 분
  걸립니다.
